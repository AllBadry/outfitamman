...
export default function FashionHero() {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // تأتي العناصر من أماكن عشوائية وزوايا مائلة لتستقر في مكانها
      gsap.from('.collage-item', {
        y: () => gsap.utils.random(-150, 150), // موضع Y عشوائي للبداية
        x: () => gsap.utils.random(-150, 150), // موضع X عشوائي للبداية
        rotation: () => gsap.utils.random(-15, 15), // دوران عشوائي
        opacity: 0,
        scale: 0.8,
        duration: 1.8,
        ease: 'expo.out', // حركة سريعة ثم تتباطأ بنعومة
        stagger: 0.05, // تتابع زمني بين كل عنصر والآخر
      });
      ...
      const scrollTl = gsap.timeline({
        scrollTrigger: {
          ...
          scrub: 1.2, // Scrub لربط الحركة بسلاسة مع عجلة الماوس
        },
      });

      // تحريك المجموعات بسرعات مختلفة لخلق عمق بصري (3D Effect)
      ...
      scrollTl.to('.parallax-reverse', { y: 90, ease: 'none' }, 0); // يتحرك للأسفل ببطء
      
      // تأثير تلاشي وتصغير للحرف M في الخلفية
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-20 relative w-[1300px] h-[900px] origin-top scale-[0.35] sm:scale-50 md:scale-75 xl:scale-100 2xl:scale-105">
        ...
        {/* 3. البطاقة الداكنة اليسرى (تتحرك للأسفل قليلاً لعمل تباين) */}
        ...
        {/* 8. بطاقة الواجهة الكبيرة (تطير للأعلى بسرعة) */}
